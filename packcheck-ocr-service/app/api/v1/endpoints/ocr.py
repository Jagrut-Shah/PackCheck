...
@router.post(
    ...
    description="Accepts an OCRRequest payload, fetches image, runs the configured OCR provider, and returns canonical OCR results."
)
async def process_ocr(request: OCRRequest) -> OCRResult:
    """
    Main OCR processing endpoint.
    Orchestrates: Download -> OpenCV Preprocessing -> OCR Provider -> Result Mapping.
    """
